...
// This component styles individual data points for consistency.
...
const ProfileHeader = ({ user }) => (
  <div className="flex flex-col items-center text-center sm:flex-row sm:text-left sm:items-center space-y-4 sm:space-y-0 sm:space-x-6 pb-6 border-b border-gray-200">
    <img
      src={user.avatarUrl || profilePic} // Fallback to a default picture
      alt="Profile"
      className="w-24 h-24 rounded-full border-4 border-white shadow-lg object-cover"
    />
    <div>
      <h2 className="text-3xl font-bold text-gray-800">{user.ownerName ?? 'N/A'}</h2>
...
// This component lays out the user's information in a responsive grid.
const ProfileDetails = ({ user }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
    <ProfileField label="Business Name" value={user.businessName} />
    <ProfileField label="Owner Name" value={user.ownerName} />
    <ProfileField label="Email" value={user.email} />
    <ProfileField label="Phone Number" value={user.phone} />
    <ProfileField label="Business Address" value={user.businessAddress} />
    <ProfileField label="Service Area (Pincode)" value={user.serviceArea} />
    <ProfileField label="Delivery Method" value={user.deliveryMethod} />
...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 sm:p-8 bg-white shadow-lg rounded-xl mt-10">
